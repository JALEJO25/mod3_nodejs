...
describe("Integración - API Completa", () => {
  ...
  describe("2. Endpoints de Notas (Protegidos con JWT)", () => {
    let validToken; // Antes de probar las notas, necesitamos generar un token falso para pasar el authMiddleware
    ...
    test("GET /api/v1/notes debería ser exitoso si se envía Token válido (200)", async () => {
      ...
      expect(response.statusCode).toBe(200); // Al principio devolverá un array vacío si no hemos conectado a una BD real con datos
